...
def prepare():
    U.exec_command("mkdir -p /root/model /root/datasets")
    U.exec_command(f"hf download Qwen/{MODEL_NAME} --local-dir /root/model/{MODEL_NAME}")
...
def execute():
    ckpt_args = f"--hf-checkpoint /root/model/{MODEL_NAME} "

    wandb_args = (
        (
            "--use-wandb "
            "--wandb-project slime-dev "
            "--wandb-group opencua_vlm_multi_turn "
